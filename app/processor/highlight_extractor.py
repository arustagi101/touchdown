...
class HighlightExtractor:
    ...
    def __init__(self, openai_api_key: Optional[str] = None):
        ...
        self.analyzer = SportsHighlightAnalyzer(self.api_key)
        self.transcript_fetcher = YouTubeTranscriptFetcher()
    
    def extract_highlights(self, video_url: str, max_highlights: int = 20) -> VideoAnalysis:
        """
        ...
        formatted_transcript = self.transcript_fetcher.format_transcript_for_analysis(transcript_entries)
        total_duration = self.transcript_fetcher.get_total_duration(transcript_entries)
        
        print(f"Video duration: {total_duration:.1f} seconds ({total_duration/3600:.1f} hours)")
        
        # Validate video duration (up to 4 hours as requested)
        if total_duration > 14400:  # 4 hours in seconds
            raise ValueError("Video is longer than 4 hours, which exceeds the supported limit")
        
        # Step 3: Analyze transcript for highlights
        try:
            print("Analyzing transcript for sports highlights...")
            highlights = self.analyzer.analyze_transcript(formatted_transcript, max_highlights)
    ...
    def extract_highlights_to_json(self, video_url: str, max_highlights: int = 20) -> dict:
        """
